...
try:
    ...
    print(
        f"health endpoint:        {'PASS' if results['health'] else 'FAIL'} - {data.get('status')}"
    )
...
try:
    ...
    print(
        f"tasks endpoint (3):     {'PASS' if results['tasks_count'] else 'FAIL'} - found {len(tasks)}"
    )
except Exception as e:
...
try:
    ...
    obs = r.json()
    easy_session = obs.get("session_id")
    results["reset_easy"] = obs.get("task_id") == "easy" and obs.get("step_count") == 0
    print(
        f"reset easy:             {'PASS' if results['reset_easy'] else 'FAIL'} - status {r.status_code}"
    )
except Exception as e:
...
try:
    r = httpx.post(
        f"{base}/step",
        params={"session_id": easy_session},
        json={
            "type": "run_sql",
    ...
    print(
        f"easy correctness=1.0:   {'PASS' if results['easy_correctness'] else 'FAIL'} - got {correctness}"
    )
except Exception as e:
...
try:
    r = httpx.get(f"{base}/grader", params={"session_id": easy_session})
    score = r.json().get("score", 0)
    results["grader"] = score > 0.9
    print(
        f"grader score>0.9:       {'PASS' if results['grader'] else 'FAIL'} - got {score}"
    )
except Exception as e:
...
try:
    r = httpx.post(
        f"{base}/reset", json={"task_id": "medium", "scenario": "inner_join"}
    )
    obs = r.json()
    medium_session = obs.get("session_id")
...
try:
    r = httpx.post(
        f"{base}/step",
        params={"session_id": medium_session},
        json={
            "type": "run_sql",
    ...
    print(
        f"medium correctness=1.0: {'PASS' if results['medium_correctness'] else 'FAIL'} - got {correctness}"
    )
except Exception as e:
...
try:
    r = httpx.post(
        f"{base}/reset", json={"task_id": "hard", "scenario": "correlated_subquery"}
    )
    obs = r.json()
    hard_session = obs.get("session_id")
...
try:
    r = httpx.post(
        f"{base}/step",
        params={"session_id": hard_session},
        json={
            "type": "run_sql",
    ...
    print(
        f"hard correctness=1.0:   {'PASS' if results['hard_correctness'] else 'FAIL'} - got {correctness}"
    )
except Exception as e:
...
try:
    ...
    print(
        f"safety filter:          {'PASS' if results['safety'] else 'FAIL'} - {error}"
    )
except Exception as e:
...
try:
    ...
    print(
        f"wrong answer low score: {'PASS' if results['wrong_answer'] else 'FAIL'} - reward={wrong_reward}"
    )
except Exception as e:
